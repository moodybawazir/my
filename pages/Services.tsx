...
import {
  ...
  Coffee, ArrowLeft,
  ShoppingCart, Calculator, Package, Printer,
  Stethoscope
} from 'lucide-react';
...
const IconMap: any = {
  Building2, BrainCircuit, CalendarDays, Coffee, ShoppingCart, Calculator, Package, Printer, Stethoscope
};
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {services.map((cat) => {
            ...
            return (
              <Link
                key={cat.id}
                to={`/service/${cat.category || cat.id}`}
              ...
              >
                ...
                <div className="pt-8 border-t border-white/5 flex items-center gap-3 text-[#cfd9cc] font-black text-lg group-hover:gap-6 transition-all">
                  مشاهدة التفاصيل <ArrowLeft size={24} />
                </div>
              </Link>
            );
          })}
