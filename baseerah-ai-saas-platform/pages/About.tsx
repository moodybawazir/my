...
const About: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('baseerah_about_content');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-48">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
            {[
              { 
                t: 'هندسة الأتمتة السيادية', 
                d: 'نصمم حلولاً تضمن بقاء بياناتك وذكاء منشأتك تحت سيطرتك الكاملة وبأعلى معايير الأمن.', 
                i: Cpu 
              },
              { 
                t: 'أمان عسكري المستوى', 
                d: 'بروتوكولات التشفير لدينا متوافقة مع أرقى المعايير العالمية لحماية المعلومات الحساسة.', 
                i: Shield 
              },
              { 
                t: 'تجربة مستخدم غامرة', 
                d: 'لا نقدم مجرد أدوات، بل نصمم تجارب بصرية وتقنية تذهل عملاءك وتسهل عملياتك.', 
                i: Video 
              },
            ].map((item, i) => (
              <div key={i} className="group relative">
                <div className="w-28 h-28 bg-[#1e403a]/40 rounded-[40px] flex items-center justify-center mx-auto mb-10 border border-[#cfd9cc]/10 group-hover:scale-110 group-hover:bg-[#cfd9cc] transition-all duration-700 shadow-xl">
                  <item.i className="text-[#cfd9cc] group-hover:text-[#0d2226]" size={48} />
                </div>
                <h3 className="text-3xl font-black text-white mb-6 tracking-tight">{item.t}</h3>
                <p className="text-[#cfd9cc]/40 leading-relaxed text-xl font-light">{item.d}</p>
              </div>
            ))}
          </div>
        </div>
