...
const UserPortal: React.FC = () => {
  ...
  const activeSubscriptions = [
    { id: 'S1', name: 'باقة برو الذكية', status: 'active', daysLeft: 22, price: 499, startDate: '١٥ يناير ٢٠٢٤', endDate: '١٥ فبراير ٢٠٢٤' },
    { id: 'S2', name: 'مساعد AI تجريبي', status: 'trial', daysLeft: 5, price: 0, startDate: '٢٨ يناير ٢٠٢٤', endDate: '٢ فبراير ٢٠٢٤' },
  ];

  const purchasedProducts = [
    { id: 'P1', name: 'جولة افتراضية - فيلا الملقا', price: 1500, status: 'completed', date: '١٢ يناير ٢٠٢٤' },
    { id: 'P2', name: 'نظام إدارة المواعيد', price: 3500, status: 'pending', date: '٢٥ يناير ٢٠٢٤' },
  ];
    ...
    <div className="min-h-screen pt-32 pb-24 px-6 bg-[#0d2226]" dir="rtl">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16">
          <div>
            <h1 className="text-4xl font-black text-white mb-3 tracking-tight">أهلاً بك مجدداً، سارة السبيعي.</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
          {[
            ...
            { label: 'إجمالي المدفوعات', val: '٥,٤٩٩ ريال', icon: CreditCard, color: 'text-white' },
            { label: 'الأيام المتبقية', val: '٢٢ يوم', icon: Clock, color: 'text-amber-400' },
          ].map((stat, i) => (
                  ...
                  <tbody className="divide-y divide-white/5">
                    {activeSubscriptions.map((sub) => (
                      <tr key={sub.id} className="group hover:bg-white/5 transition-colors">
                        ...
                        <td className="py-6 font-bold text-white">{sub.daysLeft} يوم</td>
                        <td className="py-6 text-[#cfd9cc]/40 text-sm">{sub.endDate}</td>
                        <td className="py-6">
              ...
              <div className="space-y-6">
                 {[1,2,3].map(i => (
                   <div key={i} className="flex justify-between items-center border-b border-white/5 pb-4 last:border-0 last:pb-0">
                      <div>
                        <div className="text-white font-bold text-sm">فاتورة #INV-00{i}</div>
                        <div className="text-[10px] text-[#cfd9cc]/30 uppercase font-black tracking-widest">١٥ يناير ٢٠٢٤</div>
                      </div>
                      <div className="text-left">
                        <div className="text-white font-black text-sm">٤٩٩ ريال</div>
