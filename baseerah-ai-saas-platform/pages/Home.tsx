...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const savedHome = localStorage.getItem('baseerah_home_content');
    const savedServices = localStorage.getItem('baseerah_services_data');
    if (savedHome) setCmsData(JSON.parse(savedHome));
    if (savedServices) setServices(JSON.parse(savedServices).slice(0, 3));
    else setServices([
      ...
      { id: 'real-estate', title: 'عقارات بصيرة الرقمية', icon: 'Building2', desc: 'حلول بصرية وبرمجية متطورة لقطاع العقارات الفاخرة.' }
    ]);
  }, []);
    ...
    <div className="relative overflow-hidden min-h-screen" dir="rtl">
      ...
      <section className="relative pt-48 md:pt-64 pb-24 px-6 text-center z-10">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-5 md:gap-8 px-4 animate-in fade-in duration-1000 delay-500">
          ...
          <Link to="/portal" className="glass border border-white/10 text-white px-12 py-5 rounded-2xl text-xl font-bold hover:bg-white/5 transition-luxury backdrop-blur-lg flex items-center justify-center">
            دخول العميل
          </Link>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
            {services.map((p, i) => (
              <div key={p.id} className="glass p-10 rounded-[45px] border-white/5 card-hover transition-luxury group relative overflow-hidden backdrop-blur-md">
                ...
                <Link to={`/project/${p.id}`} className="inline-flex items-center gap-3 text-[#cfd9cc] font-black text-sm group-hover:gap-6 transition-luxury">
      ...
      <section className="py-24 md:py-40 px-6 z-10 relative">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 md:gap-32 items-center">
          <div className="grid grid-cols-2 gap-4 md:gap-8 order-2 lg:order-1">
            {[
              ...
              <div key={i} className="glass p-8 md:p-12 rounded-[40px] border-white/5 flex flex-col items-center text-center group hover:bg-[#cfd9cc]/5 transition-luxury">
                <stat.icon size={32} className="text-[#cfd9cc] mb-5 group-hover:scale-125 transition-luxury" />
          ...
          <div className="space-y-10 text-right order-1 lg:order-2">
            <h2 className="text-5xl md:text-7xl lg:text-8xl font-black text-white leading-tight">بصيرة تعني <br /><span className="text-gradient">التحكم المطلق.</span></h2>
